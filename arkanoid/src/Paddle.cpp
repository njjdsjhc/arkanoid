...
Paddle::Paddle() { 
    ...
    shape_.setPosition((WINDOW_W - PADDLE_W) * HALF, PADDLE_Y);
}

void Paddle::update(float dt) { 
    float x = shape_.getPosition().x; 

    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left))  x -= PADDLE_SPEED * dt;
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) x += PADDLE_SPEED * dt;
    x = std::clamp(x, 0.f, (float)WINDOW_W - shape_.getSize().x); 
...
void Paddle::draw(sf::RenderWindow& win) const { 
    win.draw(shape_);
    if (floorActive_) { 
        sf::RectangleShape fl(sf::Vector2f((float)WINDOW_W, FLOOR_THICKNESS));
        fl.setPosition(0.f, (float)WINDOW_H - FLOOR_THICKNESS);
